...
public class Volunteer {
    ...
    @NotNull
    @Column(name = "volunteer_local_council")
    String localCouncil;

    @NotNull
    @Column(name = "volunteer_regional")
    ...
    public boolean validateCnic(){
        if(this.volunteerCnic.matches("(\\d){5}-(\\d){7}-(\\d){1}") && !this.volunteerCnic.equals("")){
    ...
    public boolean validateEmail(){
        if(this.emailAdddress.matches("^[\\w-\\+]+(\\.[\\w]+)*@[\\w-]+(\\.[\\w]+)*(\\.[a-z]{2,})$") || this.emailAdddress.equals("")){
            return true;
        }else{
            return false;
        }
